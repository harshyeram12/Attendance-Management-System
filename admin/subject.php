...
<html lang = "eng">
	...
	<body>
		...
		<div class = "container-fluid" style = "margin-top:70px;">
			...
			<br />
			<!--<div class = "alert alert-info"><a href="atex.php">Export</a></div>-->
		<div class = "alert alert-info"><a href="third_subject.php">TY</a>
		<button type = "button" class = "btn btn-success " data-target = "#myModal" data-toggle = "modal" style="float:right;margin-top:-8px;"><span class = "glyphicon glyphicon-plus" ></span> Add new</button>
		</div>
			<div class = "alert alert-info"><a href="sy_subject.php">SY</a>
			<button type = "button" class = "btn btn-success " data-target = "#myModal" data-toggle = "modal" style="float:right;margin-top:-8px;"><span class = "glyphicon glyphicon-plus" ></span> Add new</button>
			</div>
			<div class = "alert alert-info"><a href="fy_subject.php">FY</a>
			<button type = "button" class = "btn btn-success " data-target = "#myModal" data-toggle = "modal" style="float:right;margin-top:-8px;"><span class = "glyphicon glyphicon-plus" ></span> Add new</button>
			</div>
						...
						<form method = "POST" action = "save_subject_query.php" enctype = "multipart/form-data">
							<div class  = "modal-body">
								<div class = "form-group">
									...
									<input type = "text" required = "required" name = "scode" class = "form-control" />
								</div>
								<div class = "form-group">
									<label>Subject Name:</label>
									<input type = "text" maxlength = "12" required = "required" name = "sname" class = "form-control" />
								</div>
								<div class = "form-group">
									<label>Course:</label>
			...
			<div class = "well col-lg-12">
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
					<?php
						$q_time = $conn->query("SELECT * FROM `subject`") or die(mysqli_error());
						while($f_time = $q_time->fetch_array()){
							
					?>	
						<tr>
							<td><?php echo $f_time['subject_code']?></td>
							<td><?php echo htmlentities($f_time['subject_name'])?></td>
							<td><?php echo   $f_time['course'];?></td>
							<td><?php echo $f_time['teacher'];?></td>
							<td><a class = "btn btn-success rstudent_id" name = "<?php echo $f_time['subject_code']?>" href = "#" data-toggle = "modal" data-target = "#delete"><span class = "glyphicon glyphicon-remove"></span></a> <a class = "btn btn-primary  estudent_id" name = "<?php echo $f_time['subject_code']?>" href = "#" data-toggle = "modal" data-target = "#edit_student"><span class = "glyphicon glyphicon-edit"></span></a></td>
						</tr>
					<?php
						}
					?>	
		...
		$(document).ready(function(){
			$('.rstudent_id').click(function(){
				...
				$('.remove_id').click(function(){
					window.location = 'delete_subject.php?subject_code=' + $student_id;
				});
			});
			$('.estudent_id').click(function(){
				$student_id = $(this).attr('name');
				$('#edit_query').load('load_edit_subbject.php?subject_code=' + $student_id);
			});
		});
	</script>
</html>